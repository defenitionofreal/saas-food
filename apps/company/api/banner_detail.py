...
class BannerDetailAPIView(APIView):
    ...
    def get(self, request, pk, banner_pk):
        query = get_object_or_404(Banner.objects, institution_id=pk, pk=banner_pk)
        serializer = BannerSerializer(query)
        return Response(serializer.data)

    def put(self, request, pk, banner_pk):
        query = get_object_or_404(Banner.objects, institution_id=pk, pk=banner_pk)
        serializer = BannerSerializer(query, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def delete(self, request, pk, banner_pk):
        query = get_object_or_404(Banner.objects, institution_id=pk, pk=banner_pk)
        query.delete()
