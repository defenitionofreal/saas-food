...
class AdditiveDetailAPIView(APIView):
    ...
    def get(self, request, pk, additive_pk):
        query = get_object_or_404(Additive.objects, institution_id=pk, pk=additive_pk)
        serializer = AdditiveSerializer(query)
        return Response(serializer.data)

    def put(self, request, pk, additive_pk):
        query = get_object_or_404(Additive.objects, institution_id=pk, pk=additive_pk)
    ...
    def delete(self, request, pk, additive_pk):
        query = get_object_or_404(Additive.objects, institution_id=pk, pk=additive_pk)
        query.delete()
