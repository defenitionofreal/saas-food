...
from apps.order.models import Cart, CartItem, PromoCode, PromoCodeUser
            ...
            if coupon.is_active is True:
                ...
                if coupon.cart_total is not None:
                    if cart.get_total_cart < coupon.cart_total:
                        return Response({
                            "detail": f"Total cart price have to be more {coupon.cart_total}"})
                ...
                if coupon.date_start is not None:
                    if coupon.date_start > today:
                        return Response(
                            {"detail": f"Code period not started yet"})
