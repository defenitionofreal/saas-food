...
def _fill_digits_space(number: int, space_amount: int) -> str:
    return str(number).zfill(space_amount)


def _generate_random_letters(letters_amount: int) -> str:
    return "".join(random.choices(string.ascii_uppercase, k=letters_amount))


def _generate_next_order_code_number(order: Optional[Cart]) -> int:
    number = 1
    number += int(order.code.split('-')[-1]) if order else 0
    return number


def generate_order_number(institution_id_: int):
    """
    Function generates numbers like T-614 and so on.
    Number stays unique for institution and won't repeat on the same day.
    """
    last_order_code = Cart.objects.filter(
        created_at__date=datetime.date.today(),
        institution_id=institution_id_,
        code__isnull=False
    ).only("code").order_by("-code").first()
    number = _generate_next_order_code_number(last_order_code)

    return f"{_generate_random_letters(1)}-{_fill_digits_space(number, 3)}"
